...
do
curl http://localhost:9000/login     -d "username=admin"    -d "password=admin"    -d "appcode=1234567890"  > output.txt
...
# Admin collection
curl -X POST http://localhost:9000/admin/collection/Admin \
     -H X-BB-SESSION:$BBsession

curl -X POST http://localhost:9000/document/Admin \
...
# Device collection
curl -X POST http://localhost:9000/admin/collection/Device \
     -H X-BB-SESSION:$BBsession

curl -X POST http://localhost:9000/document/Device \
...
# Master collection
curl -X POST http://localhost:9000/admin/collection/Master \
     -H X-BB-SESSION:$BBsession

curl -X POST http://localhost:9000/document/Master \
...
# Profile collection
curl -X POST http://localhost:9000/admin/collection/Profile \
     -H X-BB-SESSION:$BBsession

curl -X POST http://localhost:9000/document/Profile \
...
# Version collection
curl -X POST http://localhost:9000/admin/collection/Version \
     -H X-BB-SESSION:$BBsession

curl -X POST http://localhost:9000/document/Version \
     -d '{  "number": "0.4.2"}' \
     -H Content-type:application/json \
     -H X-BB-SESSION:$BBsession
